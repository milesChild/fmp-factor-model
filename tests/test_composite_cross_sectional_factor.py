...
@pytest.fixture
def valid_characteristics(valid_vectors, valid_configs):
    """Create a list of valid characteristics."""
    return [
        CrossSectionalCharacteristic(vec, config)
        for vec, config in zip(valid_vectors, valid_configs)
    ]

@pytest.fixture
def equal_weight_characteristics(valid_vectors):
    """Create characteristics with no explicit weights."""
    return [
        CrossSectionalCharacteristic(vec, CharacteristicConfig(name=f"Char{i+1}"))
        for i, vec in enumerate(valid_vectors)
    ]

class TestCompositeCrossSectionalFactorInitialization:
    """Tests for CompositeCrossSectionalFactor initialization."""
    
    def test_valid_initialization(self, valid_characteristics):
        """Test initialization with valid characteristics."""
        factor = CompositeCrossSectionalFactor(valid_characteristics)
    ...
    def test_equal_weight_assignment(self, equal_weight_characteristics):
        ...
        factor = CompositeCrossSectionalFactor(equal_weight_characteristics)
...
class TestLengthValidation:
    ...
    @pytest.fixture
    def mismatched_length_char(self, valid_vectors, valid_configs):
        """Create a characteristic with different length."""
        different_length = np.array([1.0, 2.0, 3.0])  # Shorter length
        return CrossSectionalCharacteristic(different_length, valid_configs[0])
    
    def test_mismatched_lengths(self, valid_characteristics, mismatched_length_char):
        """Test initialization with mismatched lengths."""
        invalid_chars = valid_characteristics[:1] + [mismatched_length_char]
        with pytest.raises(ValueError, match=".*mismatched length*"):
            CompositeCrossSectionalFactor(invalid_chars)
...
class TestWeightValidation:
    ...
    @pytest.fixture
    def characteristics_with_weights(self, valid_vectors, mock_config_class):
        """Create characteristics with specific weights."""
        def _make_chars(weights):
            return [
                CrossSectionalCharacteristic(
                    valid_vectors[i], 
    ...
    def test_normalize_valid_weights(self, characteristics_with_weights):
        ...
        factor = CompositeCrossSectionalFactor(chars)
    ...
    def test_negative_weights(self, characteristics_with_weights):
        ...
        with pytest.raises(ValueError, match="Characteristic weights must be non-negative"):
            CompositeCrossSectionalFactor(chars)
...
class TestLoadingProcessing:
    ...
    def mock_zscore(self):
        ...
        with patch('src.factors.composite_cross_sectional_factor.z_score') as mock:
    ...
    def test_weighted_sum_calculation(self):
        ...
        class MockCharacteristic(CrossSectionalCharacteristic):
        ...
        with patch('src.factors.composite_cross_sectional_factor.z_score') as mock_zscore:
            ...
            pd.testing.assert_series_equal(
                factor._loadings,
                pd.Series(data=expected_final, name="Composite Factor")
            )
    ...
    def test_error_handling(self):
        ...
        class FailingCharacteristic(CrossSectionalCharacteristic):
            def get_loadings(self):
                return None
